...
*    
* See flags/parms.c for further explanations. The update program moreover
* assumes that the current deflation subspace was previously initialized
* using dfl_modes().
*
...
*
* All these are set by set_dfl_gen_parms().
*
...
* simultaneously on all processes. The required workspaces are
*
...
static void dfl_smooth_fields(spinor **mds,int *status)
{
   ...
   for (k=0;k<Ns;k++)
   {
      ...
      ltl_gcr(dgn.nkv,dgn.nmx,dgn.res,dgn.mu,wvd[0],wvd[0],&stat);
