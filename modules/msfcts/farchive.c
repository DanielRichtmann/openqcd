...
*
*   void write_flds(char *out,int n,double **f)
...
*
*   void read_flds(char *in,int n,double **f)
*     Reads the fields f[0],..,f[n-1] from the file "in" assuming they were
*     written to the file by the program write_flds().
*
*   void export_flds(char *out,int n,double **f)
...
*
*   void import_flds(char *in,int n,double **f)
*     Reads the fields f[0],..,f[n-1] from the file "in" on process 0,
*     assuming they were written to the file by the program export_flds().
*
*   void blk_export_flds(int *bs,char *out,int n,double **f)
...
*
*   void blk_import_flds(char *in,int n,double **f)
*     Performs the same operation as import_flds(), but expects the file
*     "in" to contain fields written by the program blk_export_flds().
...
* of the block. The export_flds() function does the same with B0,..,B3 set
...
*
* The block sizes bs passed to the program blk_export_flds() must divide the
...
* read the header data on the files generated by blk_export_flds() as in the
* case of the block-exported gauge-field configurations [archive/archive.c].
*
* All programs in this module perform global operations and must be called
...
static void alloc_fbuf(int n)
{
   ...
   if (ns==0)
   {
      ...
      endian=endianness();
      error_root(iup[0][0]==0,1,"alloc_fbuf [farchive.c]",
...
static double square_norm(int icom,double *f)
{
   ...
   for (;f<fm;f+=8)
   {
      sn=f[0]*f[0]+f[1]*f[1]+f[2]*f[2]+f[3]*f[3]+
...
void write_flds(char *out,int n,double **f)
{
   ...
   error_root(n<1,1,"write_flds [farchive.c]",
   ...
   if (NPROC>1)
   {
      ...
      error(iprms[0]!=n,1,"write_flds [farchive.c]",
...
void read_flds(char *in,int n,double **f)
{
   ...
   error_root(n<1,1,"read_flds [farchive.c]",
              "Parameter n is out of range");
   ...
   if (NPROC>1)
   {
      ...
      error(iprms[0]!=n,1,"read_flds [farchive.c]",
            "Parameters are not global");
   }
...
static void get_fld(int iy,int n,double **f)
{
   int y3,ix,i;
   double *fb;
...
static void set_fld(int iy,int n,double **f)
{
   int y3,ix,i;
   double *fb;
...
void export_flds(char *out,int n,double **f)
{
   ...
   error_root(n<1,1,"export_flds [farchive.c]",
              "Parameter n is out of range");
   ...
   if (NPROC>1)
   {
      ...
      error(iprms[0]!=n,1,"export_flds [farchive.c]",
            "Parameters are not global");
   }
   ...
   {
      fout=fopen(out,"wb");
      error_root(fout==NULL,1,"export_flds [farchive.c]",
                 "Unable to open output file");
      ...
      error_root(iw!=(5+n),1,"export_flds [farchive.c]",
                 "Incorrect write count");
   }
   ...
   if (my_rank==0)
   {
      error_root(iwa!=0,1,"export_flds [farchive.c]",
                 "Incorrect write count");
      fclose(fout);
   }
}
...
void import_flds(char *in,int n,double **f)
{
   ...
   error_root(n<1,1,"import_flds [farchive.c]",
              "Parameter n is out of range");
   ...
   if (NPROC>1)
   {
      ...
      error(iprms[0]!=n,1,"import_flds [farchive.c]",
            "Parameters are not global");
   }
   ...
   {
      fin=fopen(in,"rb");
      error_root(fin==NULL,1,"import_flds [farchive.c]",
                 "Unable to open input file");

      ir=fread(lsize,sizeof(stdint_t),5,fin);
      error_root(ir!=5,1,"import_flds [farchive.c]",
                 "Incorrect read count");
      ...
      error_root(ie!=0,1,"import_flds [farchive.c]",
                 "Unexpected lattice sizes or number of fields");

      ir=fread(snrm,sizeof(double),n,fin);
      error_root(ir!=n,1,"import_flds [farchive.c]",
                 "Incorrect read count");
   ...
   if (my_rank==0)
   {
      error_root(ira!=0,1,"import_flds [farchive.c]",
                 "Incorrect read count");
      fclose(fin);
   }
...
void blk_export_flds(int *bs,char *out,int n,double **f)
{
   ...
   error_root(n<1,1,"blk_export_flds [farchive.c]",
              "Parameter n is out of range");
   ...
   if (NPROC>1)
   {
      ...
      error((iprms[0]!=bs[0])||(iprms[1]!=bs[1])||(iprms[2]!=bs[2])||
            (iprms[3]!=bs[3])||(iprms[4]!=n),1,"blk_export_flds [farchive.c]",
            "Parameters are not global");
   }
   ...
   nios=nio_streams();
   error_root((nios<1)||((nb%nios)!=0),1,"blk_export_flds [farchive.c]",
              "Improper number of parallel I/O streams");
   m=nb/nios;
   ...
   for (i=0;i<m;i++)
   {
      if (i==(ib%m))
      {
         ...
         {
            fout=fopen(out,"wb");
            error_loc(fout==NULL,1,"blk_export_flds [farchive.c]",
                      "Unable to open output file");
            ...
            error_loc(iw!=(13+n),1,"blk_export_flds [farchive.c]",
                      "Incorrect write count");
         }
         ...
         if (my_rank==ip0)
         {
            error_loc(iwa!=0,1,"blk_export_flds [farchive.c]",
                      "Incorrect write count");
            fclose(fout);
         }
      }
...
void blk_import_flds(char *in,int n,double **f)
{
   ...
   error_root(n<1,1,"blk_import_flds [farchive.c]",
              "Parameter n is out of range");
   ...
   if (NPROC>1)
   {
      ...
      error(iprms[0]!=n,1,"blk_import_flds [farchive.c]",
            "Parameters are not global");
   }
   ...
   error_root((nl[0]!=N0)||(nl[1]!=N1)||(nl[2]!=N2)||(nl[3]!=N3),1,
              "blk_import_flds [farchive.c]","Unexpected lattice sizes");

   ip0=blk_root_process(nl,bs,bo,&nb,&ib);
   nios=nio_streams();
   error_root((nios<1)||((nb%nios)!=0),1,"blk_import_flds [farchive.c]",
              "Improper number of parallel I/O streams");
   m=nb/nios;
   ...
   for (i=0;i<m;i++)
   {
      if (i==(ib%m))
      {
         ...
         {
            fin=fopen(in,"rb");
            error_loc(fin==NULL,1,"blk_import_flds [farchive.c]",
                      "Unable to open input file");

            ir=fread(lsize,sizeof(stdint_t),13,fin);
            error_loc(ir!=13,1,"blk_import_flds [farchive.c]",
                      "Incorrect read count");
            ...
            error_loc(ie!=0,1,"blk_import_flds [farchive.c]",
                      "Unexpected file header data or number of fields");

            ir=fread(snrm,sizeof(double),n,fin);
            error_loc(ir!=n,1,"blk_import_flds [farchive.c]",
                      "Incorrect read count");
         ...
         if (my_rank==ip0)
         {
            error_loc(ira!=0,1,"blk_import_flds [farchive.c]",
                      "Incorrect read count");
            fclose(fin);
         }
      }
   ...
   error(ie!=0,1,"blk_import_flds [farchive.c]",
         "Incorrect field square norms");
}
