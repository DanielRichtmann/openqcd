...
* File check11.c
*
* Copyright (C) 2012, 2013, 2016, 2018 Martin Luescher, Stefan Schaefer
*
...
static double check_rotpf(double mu0,double mu1,int ipf,int isp)
{
   int status[3];
   double nrm,dev;
   ...
   dev+=unorm_dble(VOLUME/2,1,chi)/nrm;
...
int main(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      if (bc!=0)
         error_root(sscanf(argv[bc+1],"%d",&bc)!=1,1,"main [check111.c]",
                    "Syntax: check11 [-bc <type>] [-sw <type>]");

      if (is!=0)
         error_root(sscanf(argv[is+1],"%d",&is)!=1,1,"main [check111.c]",
                    "Syntax: check11 [-bc <type>] [-sw <type>]");
   }
