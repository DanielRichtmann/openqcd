...
* File check10.c
*
* Copyright (C) 2012-2016, 2018 Martin Luescher, Stefan Schaefer
*
...
static double check_rotpf(double mu,int ipf)
{
   ...
   restore_ranlux();
   rotpf4(mu,ipf,0,0,1.234,-1.234,NULL);

   return unorm_dble(VOLUME/2,1,phi)/nrm;
}
...
{
   int my_rank,bc,is,isw,isp,mnkv,status[6];
   ...
   if (my_rank==0)
   {
      ...
      if (bc!=0)
         error_root(sscanf(argv[bc+1],"%d",&bc)!=1,1,"main [check100.c]",
                    "Syntax: check10 [-bc <type>] [-sw <type>]");

      if (is!=0)
         error_root(sscanf(argv[is+1],"%d",&is)!=1,1,"main [check100.c]",
                    "Syntax: check10 [-bc <type>] [-sw <type>]");
   }
