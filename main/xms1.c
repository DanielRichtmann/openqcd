...
static void read_iodat0(void)
{
   ...
   if (my_rank==0)
   {
      ...
      error_root((strlen(line)!=1)||(type==0x0),1,"read_iodat0 [xms1.c]",
      ...
      error_root((first<1)||(last<first)||(step<1)||((last-first)%step!=0),1,
                 "read_iodat0 [xms1.c]","Improper configuration range");
   }
...
static void read_iodat1(void)
{
   ...
   else
   {
      if (my_rank==0)
      {
         ...
         error_root((strlen(line)!=1)||(type==0x0),1,"read_iodat1 [xms1.c]",
                    "Improper configuration storage type");
...
static void read_wflow_parms(void)
{
   ...
   if (my_rank==0)
   {
      ...
      if (strcmp(line,"EULER")==0)
         ...
         error_root(1,1,"read_wflow_parms [xms1.c]","Unknown integrator");
   }
   ...
   if (my_rank==0)
   {
      if (append)
      {
         ...
         error_root(ir!=2,1,"read_wflow_parms [xms1.c]",
                    "Incorrect read count");
         error_root(ie!=0,1,"read_wflow_parms [xms1.c]",
      ...
      else
      {
         ...
         iw=fwrite(istd,sizeof(stdint_t),1,fdat);
         iw+=fwrite(dstd,sizeof(double),1,fdat);
         error_root(iw!=2,1,"read_wflow_parms [xms1.c]",
                    "Incorrect write count");
      }
   }
}
...
static void read_obs_parms(void)
{
   ...
   if (my_rank==0)
   {
      ...
      ntm=count_tokens("tm");
      error_root(ntm==0,1,"read_obs_parms [xms1.c]",
   ...
   error(tm==NULL,1,"read_obs_parms [xms1.c]",
   ...
   if (my_rank==0)
   {
      if (append)
      {
         ...
         error_root(ir!=(4+ntm),1,"read_obs_parms [xms1.c]",
                    "Incorrect read count");
         error_root(ie!=0,1,"read_obs_parms [xms1.c]",
                    "Parameters do not match previous run");
      }
      else
      {
         ...
         error_root(iw!=(4+ntm),1,"read_obs_parms [xms1.c]",
                    "Incorrect write count");
      }
   }
}
...
static void save_iodat(void)
{
   ...
   if (my_rank==0)
   {
      if (append)
      {
         ...
         error_root(ir!=6,1,"save_iodat [xms1.c]",
                    "Incorrect read count");
         ...
         error_root(ie!=0,1,"save_iodat [xms1.c]",
                    "Parameters do not match previous run");
      }
      else
      {
         ...
         error_root(iw!=6,1,"save_iodat [xms1.c]",
                    "Incorrect write count");
      }
   }
}
...
static void check_files(void)
{
   ...
   if (ipsm==0)
   {
      ...
      else if (type==0x2)
      {
         ...
         n=nb/nion;
         error_root(nb%nion!=0,1,"check_files [xms1.c]",
      ...
      else
      {
         nion=iodat[0].nio_nodes;
         n=NPROC/nion;
         error_root(NPROC%nion!=0,1,"check_files [xms1.c]",
   ...
   if (noflds==0)
   {
      ...
      if (type==0x1)
      {
         error(name_size("%s/%sn%d.fld",cnfg_dir,nbase,last)>=NAME_SIZE,1,
               "check_files [xms1.c]","flds_dir name is too long");
         check_dir_root(cnfg_dir);
      ...
      else if (type==0x2)
      {
         if (append)
         {
            ...
            error_root((bs[0]!=iodat[1].bs[0])||(bs[1]!=iodat[1].bs[1])||
                       ...
                       "check_files [xms1.c]","I/O block size mismatch");
         }
         else
         {
            ns[0]=N0;
...
static void set_data(int idat)
{
   ...
   for (i=0;i<ntm;i++)
   {
      data.E[i]=center_fld(fE[i]);

      for (j=0;j<=i;j++)
      {
         if (i3d)
            cov3d_fld(dmax,fE[i],fE[j],rf[0],rf[1],f[0],var[i][j]);
         else
            cov_fld(dmax,fE[i],fE[j],rf[0],rf[1],f[0],var[i][j]);
...
int main(int argc,char *argv[])
{
   ...
   if (flint)
      alloc_wfd(1);
   alloc_flds(1+2*file_head.ntm,2);
