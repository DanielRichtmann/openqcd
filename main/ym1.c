...
* File ym1.c
*
* Copyright (C) 2010-2019 Martin Luescher
*
...
static void read_dirs(void)
{
   ...
   if (my_rank==0)
   {
      if (scnfg)
      {
         ...
         error_root(type==0x0,1,"read_dirs [ym1.c]",
...
static void read_wflow_parms(void)
{
   ...
   if (my_rank==0)
   {
      ...
      if (noms==0)
      {
         ...
         error_root((dn<1)||(nt<dn)||((nt%dn)!=0),1,"read_wflow_parms [ym1.c]",
...
static void save_iodat(void)
{
   ...
   if (my_rank==0)
   {
      if (append)
      {
         ...
         error_root(ir!=6,1,"save_iodat [ym1.c]",
                    "Incorrect read count");
         ...
         error_root(ie!=0,1,"save_iodat [ym1.c]",
                    "Parameters do not match previous run");
      }
      else
      {
         ...
         error_root(iw!=6,1,"save_iodat [ym1.c]",
                    "Incorrect write count");
      }
   }
}
...
static void check_files(int *nl,int *icnfg)
{
   ...
   if (scnfg)
   {
      ...
      if (type&0x1)
      {
         error(name_size("%s/%s",init_dir,cnfg)>=NAME_SIZE,1,
               "check_files [ym1.c]","init_dir name is too long");
      ...
      else if (type&0x2)
      {
         ...
         n=nb/nion;
         error_root(nb%nion!=0,1,"check_files [ym1.c]",
   ...
   if (type&0x2)
   {
      ...
      error(name_size("%s/%d/%d/%sn%d_b%d",blk_dir,nion-1,n-1,nbase,
            ...
            "check_files [ym1.c]","block_dir name is too long");
   ...
   if (type&0x4)
   {
      nion=iodat[1].nio_nodes;
      n=NPROC/nion;
      error_root(NPROC%nion!=0,1,"check_files [ym1.c]",
                 "Number of processes is not a multiple of nio_nodes");
...
static void save_flds(int icnfg)
{
   ...
   error_root(ie!=0,1,"save_flds [ym1.c]",
