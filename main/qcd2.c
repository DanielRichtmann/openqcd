...
static void read_lat_parms(void)
{
   ...
   lat=set_lat_parms(beta,c0,kappa[0],kappa[1],kappa[2],csw[0],csw[1],csw[2]);
...
static void read_schedule(void)
{
   ...
   if (my_rank==0)
   {
      ...
      {
         ir=fread(istd,sizeof(stdint_t),3,fdat);         
         error_root(ir!=3,1,"read_schedule [qcd2.c]",
                    "Incorrect read count");         
      ...
      else
      {
         ...
         iw=fwrite(istd,sizeof(stdint_t),3,fdat);         
         error_root(iw!=3,1,"read_schedule [qcd2.c]",
                    "Incorrect write count");
      }
   }
}
...
static void read_infile(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      error_root((noexp)&&(noloc),1,"read_infile [qcd2.c]",
...
static void check_old_dat(int nl)
{
   ...
   error_root(nt!=nl,1,"check_old_dat [qcd2.c]",
...
static void check_old_msdat(int nl)
{
   ...
   for (ic=0;;ic++)
   {
      ...
      {
         dnt=nt-pnt;
         error_root(dnt<1,1,"check_old_msdat [qcd2.c]",
                    "Incorrect trajectory separation");
      }
      else if (ic>1)
         error_root(nt!=(pnt+dnt),1,"check_old_msdat [qcd2.c]",
   ...
   error_root((nt!=nl)||((ic>1)&&(dnt!=dtr_ms)),1,
              "check_old_msdat [qcd2.c]","Last trajectory numbers "
   ...
   if (my_rank==0)
   {
      if (noloc)
         error_root(cnfg[strlen(cnfg)-1]=='*',1,
                    "check_files [qcd2.c]","Attempt to read an "
      ...
      if (noloc==0)
         error_root(name_size("%s/%sn%d_%d",loc_dir,nbase,icmax,NPROC-1)>=
                    NAME_SIZE,1,"check_files [qcd2.c]",
...
static void init_ud(void)
{
   ...
   if (scnfg)
   {
      ...
      {
         ie=check_sfbcd();
         error_root(ie!=1,1,"init_ud [qcd2.c]",
